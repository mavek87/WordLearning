    ...
    public boolean isTranslationForVocableEqualsToStringPresent(Vocable vocable, String string) {
        ...
        try (Connection connection = Database.getInstance().getConnection();
            ...
            if (resultSetTranslations.next()) {
                ...
                LOG.debug("SELECT COUNT (*) FROM translations AS t LEFT JOIN DictionaryTranslations dt ON t.Id=dt.Translation_Id WHERE dt.Vocable_Id=" + vocable.getId() + " AND t.Translation='" + string + "' COLLATE NOCASE ORDER BY t.Translation ASC");
            }
        } catch (SQLException ex) {
            LOG.error(ex.getMessage());
            Database.getInstance().printSQLException(ex);
